...
- **YELLOW sweep in progress (session 36+):** H4, H5, H6, H8, H9, H7, H11, H13, H14, H15 done (H7 resolved session 39 — "Apache-2.0 strays" premise was stale drift; docs already GPL-3.0, normalized `Licence`→`License` + `Version 3`→`v3`. H11 resolved session 40 — Flutter/Dart canonical confirmed live in `ui/flutter_ui/`; Kivy drift fixed across `setup.py`/`kernel/requirements.txt`/`kernel/gui.py`/`start_gui_shell`/README; H22/H25/H122/H125 closed. H13 resolved session 41 — `.umerpkg` real Ed25519 signing + pinned chain-of-trust; was overstated crypto. H14 resolved session 42 — README reconciled. H15 resolved session 43 — requirements.txt: g4f removed, 3 `>` floats bounded `<1.0`, kivy removal explicit, ruff+mypy already enforced, reproducibility policy + `requirements.lock` added). Next = **H16** (`tests/` harness framework split + no CI suite step + no root pytest config; pick ONE framework, add CI `pytest` step, make suite discoverable & green, H9), then remaining YELLOW.
