...
- Session 23 (legal/ GPL-3.0 canonical + consent fail-closed cluster): H128 (`licenses.py` docstring + `README.md:40` now GPL-3.0-exclusive, no Apache/GPL-2.0/MIT multi-license framing); H129 (`scan_directory` tightened to require an explicit GPL-3.0 declaration — canonical header / `License: GPL-3.0` / SPDX id — removing the loose `"GPL-3.0"` substring fail-open; was already `[x]` session 3, tightened further); H130 (`get_license_text` raises `ValueError` on non-GPL, no silent Apache substitution); H131 (`require_consent_interactive` fail-closed — no auto-grant in dry_run/non-TTY, raises `ConsentGateError` unless `allow_non_interactive=True`); H135 (`consent` CLI requires `--i-agree` or real TTY "I AGREE", no longer hardcodes `user_response="I AGREE"`; `legal/test_legal.py` updated). 13 new security tests in `tests/test_legal_security.py` (TestLicenseAuditFailClosed/TestGetLicenseTextStrict/TestConsentGateFailClosed/TestConsentCliFailClosed); full suite 1460 passed / 0 failures / 0 errors (excl. pre-existing `test_ai.py` collection error).
