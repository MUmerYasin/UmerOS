...
## NEXT — YELLOW sweep (session 40): H7 RESOLVED (session 39), **H11 RESOLVED (session 40)** — UI reconciled: Flutter/Dart canonical confirmed LIVE in `ui/flutter_ui/`; Kivy drift fixed across `setup.py` [all] extras (legacy note), `kernel/requirements.txt` (kivy commented RETIRED), `kernel/gui.py` (retired banner), `kernel/umer_kernel.start_gui_shell` (docstring+comment reconciled to legacy Tkinter fallback + `[FIX H116]` marker), and 7 README Kivy refs refreshed as legacy; H22/H25/H122/H125 closed. H14/H15/H116 left open (broader scope: README layout drift / g4f+floating pins / capability-gate+Flutter launch). H9 done (session 38). Next: **H13** — security/package signing (`.umerpkg` signing + chain-of-trust must hold). Say **'continues'** for H13.
