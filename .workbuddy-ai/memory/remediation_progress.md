...
## NEXT — YELLOW sweep (session 41→43): H7 RESOLVED (39), H11 RESOLVED (40), H13 RESOLVED (41), **H14 RESOLVED (42)**, **H15 RESOLVED (session 43)** — requirements.txt: `g4f` removed (0 import sites), 3 strict `>` floats bounded `<1.0`, security/transport caps added, kivy removal made explicit, reproducibility policy + `requirements.lock` recommendation added; drift-recon confirmed (d) legacy linters already gone & README Contributing already ruff+mypy, (c) kivy already commented in reqs (H22/H25). Next: **H16** — `tests/` harness framework split (unittest vs pytest) + no CI suite step + no root pytest config (design src prescribes pytest but bundled pytest imports removed `imp`); pick ONE framework, add `pyproject [tool.pytest]` (exists) + CI `pytest` step, repin `pytest>=8`, make suite discoverable & green (H9). Say **'continues'** for H16.
