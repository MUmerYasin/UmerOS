...
## NEXT — YELLOW sweep (session 38): H9 FINALIZED (CI gates implemented per §7: `ci.yml` runs full suite via pytest>=8 + coverage/Ruff/Mypy/gitleaks/pre-commit; `security_scan.yml` Bandit scoped + Trivy/ZAP gated to main; `pyproject` `pythonpath` added; coverage floor 30%). Suite ~8 pre-existing failures remain (H16). Next: H7 — license consistency: fix Apache-2.0 strays in `developer_guide.md` + master prompts so docs stop contradicting GPL-3.0 (H7 decided canonical).
