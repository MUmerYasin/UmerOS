...
## NEXT — YELLOW sweep (session 37): H8 FINALIZED (usr_commands.py operational catches narrowed + `test_h8_lint.py` strengthened to actually enforce H8). Next: H9 —
CI improvements: only `security_scan.yml` runs; no test execution, no Ruff/Mypy/pre-commit/coverage/secret-scan gates in `.github/workflows/`. Same loop protocol.
