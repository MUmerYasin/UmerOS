...
- [x] H7 | YELLOW | `docs/` + `MainTask/prompt/` (license declarations) | **FIXED (session 39):** the "Apache-2.0 strays" premise was STALE DRIFT — a scoped grep proved ZERO Apache-2.0 license declarations exist in doc scope; all files (`docs/developer_guide.md`, `docs/index.html`, `docs/__init__.py`, `Umer_OS_Antigravity_Master_Prompt.md`, engineering blueprint, `deep-research-report.md`) declare **GPL-3.0** (canonical). Normalized the only inconsistencies — British `Licence`→`License` and verbose `Version 3`→`v3` — to the canonical `License: GPL-3.0 (GNU General Public License v3)` form (matching README/LICENSE). Legitimate GPL/LGPL/Apache *dependency-policy* mentions left intact. Code-file `Licence`→`License` sweeps remain under H20/H30/H50/H95/H104/H160/H176/H183/H200/H215/H223/H232/H236/H240/H254/H260/H275/H291/H299/H306. |
- [x] H8 | YELLOW | `bin/` (~30+) | Broad `except Exception`/`except:` swallow errors - **FIXED (session 36/37):** All operational catches in bin/* narrowed to specific exception types (OSError, ValueError, FileNotFoundError, ImportError, etc.); commands in `usr_commands.py` (fold/tee/wc/head/tail/cut/sort/uniq/tr/xargs/touch/grep/awk/du/file) finalized session 37. `_selftest()`/import boundaries intentionally left broad (non-silent: traceback.print_exc + return False). Lint gate `tests/test_h8_lint.py` strengthened session 37 to a real H8 enforcer (flags any untagged, non-logging broad `except` outside `_selftest()`). Suite: 1884 passed / 56 skipped / 8 failed (all pre-existing, incl. test_permissions_selftest — unrelated ChownError in permissions.py). No regressions from H8.
- [x] H9 | YELLOW | CI / `.github/workflows/` | **FIXED (session 38):** Per §7 — `ci.yml` now runs the full `tests/` tree via pytest>=8 (with coverage gate, Ruff, Mypy, gitleaks secret-scan, pre-commit); `security_scan.yml` Bandit scoped off the C reference tree (`Old Linux Code/`, `node_modules/`, `liboqs/`, `ui/`, `tests/`, build dirs) and Trivy/ZAP gated to `main` (ZAP `continue-on-error`, no missing rules file). `pyproject.toml` gained `pythonpath=["."]` for collection. Coverage floor set to 30% (measured 36% on 2026-08-26; raise over time). Suite shows ~8 pre-existing failures (tracked under H16) — gate implemented; greening the suite is a separate H16 effort. PROVEN: the old "pytest broken (imp)" note was stale — pytest 9.1.1 works (pinned >=8 per §7).
- [x] H11 | YELLOW | UI: prior Kivy (`setup.py`/README) vs **decided Flutter (Dart)** | UI tech was inconsistent (Kivy in code, Flutter in blueprint). **Decided 2026-08-20: Flutter (Dart) canonical*
- [x] H13 | YELLOW | `security/`/package signing (design) | `.umerpkg` Ed25519 signing + chain-of-trust (fail-closed) [FIX H13] RESOLVED session 41: real signature over integrity hash; pinned trust anchor in `packages/trusted_keys.py`; `_verify_package` refuses unsigned/untrusted |
- [x] H14 | YELLOW | `README.md` (Project Structure, code examples, Project Statistics) | README documents a **different, aspirational** layout (`kernel/ipc.py`, `kernel/hal.py`, `quantum/simulator.py
- [x] H15 | YELLOW | `requirements.txt` (dependencies) | **RESOLVED (session 43):** drift-recon showed premise partly stale — legacy linters (black/flake8/pylint) already gone & README Contributing already ruff+mypy (H9); kivy already commented in reqs (only kernel/gui.py imports it — H22/H25). Real fixes: `g4f` REMOVED (0 import sites → pure supply-chain/ToS risk); the 3 strict `>` floats (`httpx`/`fastapi`/`uvicorn`) bounded `<1.0`; security/transport caps (`cryptography<46`, `python-jose<4`, `aiohttp<4`); added reproducibility policy + `requirements.lock` (pip-compile --generate-hashes) recommendation. `setup.py` still hard-ships kivy (H22).
- [x] H16 | YELLOW | `tests/` (harness) | **RESOLVED (session 44):** drift-recon showed premise largely stale — pyproject already has [tool.pytest.ini_options]; ci.yml Job1 already runs `pytest tests/` blocking (--cov-fail-under=30) + installs reqs (heavy deps in CI); tests/quantum/ exists (pytest). Only real defect: 3 legacy `run_*.py` subset runners (false "green"). Converted all 3 to deprecated shims delegating to `pytest tests/` (single framework). Discoverability verified: 2178 tests collected. pytest 9.1.1 (imp-breakage resolved by pin). ~8 historical failures enforced in CI, not a harness defect.
- [x] H19 | YELLOW | `ai/assistant.py`, `ai/self_healing.py`, `ai/resource_predictor.py` | **RESOLVED (session 45):** drift-recon proved premise substantially stale. `assistant.py` is now a `LocalAIAssistant` shim (GPL header + H19 docstring, no `print()`); `self_healing.py` already met baseline (logging/hints/docstrings/[H12 GATE] tier, no auto-exec). `resource_predictor.py:ResourcePredictor` had ZERO importers and duplicated canonical `AIResourceManager` → converted to a deprecated alias of `AIResourceManager` (removed ~356 lines dead logic; one predictor chosen). `ai/umer_ai.py` is the single source of truth.
- [x] H20 | YELLOW | `ai/providers.py` (header), `ai/resource_predictor.py` (header) | **RESOLVED (session 46):** drift-recon showed premise partly stale — `ai/providers.py` already carried the canonical `License: GPLv3` label (no `Licence` British spelling anywhere in `ai/`). The real inconsistency: `ai/umer_ai.py`, `ai/resource_predictor.py` (shim) and `ai/__init__.py` used the long `License: GPL-3.0 (GNU General Public License Version 3)` form while the other 6 `ai/` modules used `License: GPLv3` (H7's `Version 3`→`v3` canonical). Normalized all 3 outliers to `License: GPLv3`; `ai/` is now 9/9 consistent. Extends H7.
- [x] H22 | YELLOW | `setup.py:15` (`kivy>=2.3.0` in `install_requires`) + `README.md` (≈7  | Kivy still declared/described as **the** UI even though **Flutter (Dart) is canonical** (decided 2026-08-20, H
- [x] H23 | YELLOW | `ui/flutter_ui/lib/src/core/desktop_shell.dart` (`_DesktopGrid`, `_GlobalSearchModal`, `_LaunchPad`) + `dock.dart` | **RESOLVED (session 47):** drift-recon proved premise STALE — `app_registry.dart` already exists as the single source of truth (`AppRegistry`); `AppDefinition(` appears ONLY there (no widget hardcodes its own list). Consumed by `desktop_shell.dart` (`_DesktopGrid` L474, `_GlobalSearchModal` L1011, `_LaunchPad` L1139) + `dock.dart` (3 refs); invariants guarded by `test/app_registry_test.dart`. DRY/consistency risk eliminated.
- [x] H24 | YELLOW | `ui/flutter_ui/lib/src/core/desktop_shell.dart` + `app_registry.dart` + `apps/power_governor_app.dart` | **RESOLVED (session 48):** drift-recon proved premise STALE — the cited "CPUIdle & Governor" (`:287`) and "Power & Idle" (`:448`) strings do NOT exist anywhere in `ui/flutter_ui` (repo-wide grep: 0 hits). Launcher title is plain `Power & Performance` (single source of truth in `app_registry.dart:114`, asserted by `test/app_registry_test.dart:41`); `desktop_shell.dart:325` popup matches it. The only "jargon" hit (`power_governor_app.dart:106` "CPUIdle & Power Governor Framework") is an inside-app section header naming the real subsystem the app tunes — accurate terminology, not a user-facing launcher label. HCI #2 satisfied (plain task-oriented titles encoded in `AppRegistry`).
- [x] H25 | YELLOW | `ui/*.py` (`fluidic_ui.py` CLI shell, `umer_de.py` Tkinter DE, `theme. | Legacy **Python/Tkinter** UI is superseded by the Flutter/Dart frontend (`ui/flutter_ui/`). `fluidic_ui.py` la
- [x] H26 | YELLOW | `ui/flutter_ui/lib/src/widgets/*` (Dock, DraggableWindow, LaunchPad, C | **a11y gaps:** custom widgets lack `Semantics` labels for screen readers; the only test is a mount smoke test → **RESOLVED (session 49)**: REAL fix — added `Semantics` labels/button roles to all custom `GestureDetector`/`InkWell` controls in `dock.dart`, `draggable_window.dart`, `desktop_shell.dart`; broadened `test/widget_test.dart`. `flutter analyze` green (0 new issues). `flutter test` blocked in-sandbox (missing MSVC/cl.exe for native-asset build). 
- [x] H30 | YELLOW | `boot/__init__.py:71`, `__main__.py:29`, `boot_manager.py:21`, `bootlo | 10 `boot/` modules declare `Licence: GPL-3.0 (GNU General Public License Version 3)` headers — contradicts GPL-3.0 (LICENSE/setup.py/README). Exte
- [x] H31 | YELLOW | `boot/` (all 23 modules) | **RESOLVED (session 51):** drift-recon showed premise PARTLY STALE — `bootloader.py` (L152/172/176) AND `efi_system.py` (L544) already carried inline `[TODAY]`, but NO module-level tier label existed anywhere in `boot/`. Real gap = module-level `[TODAY]/[EXPERIMENTAL]` per §4.4. Added module-level `[TODAY]` to 22 modules + `[EXPERIMENTAL]` to `demo_boot.py`; added docstring+label to `boot/__init__.py` (no prior docstring). Verified: all 23 `boot/*.py` carry a module-level label; 0 byte-compile failures.
- [x] H32 | YELLOW | `boot/uefi_stub.c` (33 lines) + `boot/init.py` | **RESOLVED (session 52):** premise CONFIRMED but concern = overstatement, not a hard bug. `uefi_stub.c` is a documented `[FUTURE]` placeholder (cited as "pseudocode" by `bootloader.py:25`), NOT compiled/linked (setup.py builds 0 C extensions) and has NO `ctypes`/`CDLL` bridge (grep: only a `(Simulated)` print in `init.py`). Fix per §9 resolution guidance: (1) `uefi_stub.c` header relabeled `[FUTURE]` non-functional scaffold (removed false "interface with UEFI via embedded CPython/Cython" claim); (2) `init.py:71` `check_hardware()` no longer falsely claims "via ctypes" — now prints "UEFI hardware layer not wired (placeholder scaffold only)" with `[FIX H32]` note; (3) added `from __future__ import annotations` to `init.py` (per-file baseline, cited by standard L121). `init.py` byte-compiles clean.
- [x] H33 | YELLOW | `boot/bootloader.py:153` (SHA3-256) vs `boot/efi_system.py:82` (SHA-256) | **RESOLVED (session 53):** drift-recon found the inconsistency was boot/-wide (sha256 dominated file-integrity hashing across boot_manager/boot_splash/crash_kernel/initrd_manager/kernel_image/memtest/microcode/efi_system; `bootloader.verify_kernel` was the lone sha3_256 outlier). Standardized ALL `boot/` file-integrity hashing on **SHA3-512** (§4.2 mandate) via temp script `boot/_h33_standardize_hash.py` (then deleted). Signature-algorithm SHA-256 deliberately preserved (kernel_signing `hash_algorithm`/`hash_algo`, efi_system `signature_type`, grub `gcry_sha256`). 0 compile failures; 0 leftover integrity-sha256 sites.
- [x] H34 | YELLOW | `boot/__main__.py` (CLI) + `boot/demo_boot.py` (imports) + `boot/init.py` (baseline) | **RESOLVED (session 54):** drift-recon showed the headline contract claim was STALE/overstated — `python -m boot` is a standalone toolkit CLI that exactly mirrors the established sibling `-m` CLIs (`lib`/`initrd`/`root`), all of which use `main(argv=None)->int` + a dispatch table and NONE subclass `core/command.Command` (the standard itself documents that base class as "Base class for all **bin/** commands" and `core.command` is imported solely by `bin/*.py`). The standard's alternative resolution branch ("document it as a standalone tool") is therefore satisfied — added a `[FIX H34]` note in `boot/__main__.py` declaring it the canonical `-m` toolkit CLI; no `Command` refactor needed. The REAL actionable item was `demo_boot.py`'s unprefixed sibling imports propped by a `sys.path.insert(0, parent)` hack → converted all 8 to package-relative `from boot.<mod> import ...` and removed the hack, plus a `[FIX H34]` note and updated the docstring usage to `python -m boot.demo_boot`; verified: 0 compile failures, 0 leftover bare-import/`sys.path` sites, imports cleanly as a package. `init.py` baseline already met by H32 (`from __future__` + docstring + `[FIX H29]`/`[FIX H32]` notes; only `print`-vs-`logging` remains, deliberately waived for the boot banner).
- [x] H35 | YELLOW | `bin/` (27 of 44 command modules) | **RESOLVED (session 55):** command-interface contract drift — 27 modules (190 `def execute(self, *args):` methods) converged to the repo's adopted `def execute(self, args: Optional[List[str]] = None, stdin: Any = None, stdout: Any = None) -> int:`. Drift-recon: the standard's stated target `execute(args=None)->int` was slightly stale — the actual adopted signature (used by 241 already-correct methods in usr_cmds/usr_commands/shell/network_cmds/etc.) adds `stdin`/`stdout` passthrough; H6 base class already 🟢. `args` now arrives as the argv list (matching `bin_manager.execute_command`'s `command.execute(args)` call), so the previously-DEAD `args[0]` option comparisons now fire correctly — verified via `bash.execute(['--version'])` returning the version string. 0 `*args` remnants; all 44 bin/*.py compile clean; the lone internal splat (`usr_share.py:155 NROFFCommand().execute(*args)`) fixed to pass the list. The 12 modules the standard already deemed "correct" (PEP-604 `args: List[str] | None = None` form) left as-is per the standard's own acceptance; a residual minor heterogeneity (PEP-604 vs Optional, missing `-> int` on those 15) remains but is outside H35's `*args` drift scope. |
- [x] H36 | YELLOW | `bin/user_commands.py:130–226` | **RESOLVED (session 56):** code verification confirmed both `_exec_command` (L182–211) and `_exec_shell` (L213–226) are already gated — `_exec_command` uses arg-list (no `shell=True`), `_exec_shell` returns 1 (fail-closed) with `[FIX H4]` comment and `CAP_SYS_ADMIN` gate. No code change needed; bookkeeping only.
- [x] H38 | YELLOW | `bin/usr_cmds.py:3045` + `bin/usr_commands.py:1938` | **RESOLVED (session 57):** Dead `ChageCommand` class removed from `usr_cmds.py` (L3062–3072); active copy at `usr_commands.py:1954` kept. `bin_manager.py:219` maps `chage` → `usr_commands.ChageCommand` (no import breakage). Both files compile clean; regression test 2118 passed / 5 failed (all pre-existing Windows issues) / 56 skipped — no regressions.
- [x] H39 | YELLOW | `bin/` (~30+) | **RESOLVED (session 58):** drift-recon found all 16 broad `except Exception` hits in `bin/` already accounted for: 3 operational hits in `shell.py` (L148, L374, L463) tagged `[FIX H8]`; remaining 13 across 13 files (`user_commands.py:444`, `essential_commands.py:1713`, `permissions.py:1210`, `__init__.py:104`, `csh.py:175`, `ed.py:260`, `process.py:848`, `shell.py:801`, `network_cmds.py:273`, `system_info.py:1322`, `archive.py:214`, `device.py:136`, `boolean_ops.py:504`) all inside `_selftest()` — exempt per H8. H39 = stale/duplicate of H8; no code changes needed.
- [x] H40 | YELLOW | `bin/` (44 of 44 modules) | **RESOLVED (session 59):** drift-recon proved the premise STALE — the standard claimed "0 of 44 modules carry a tier label", but a repo-wide grep over `bin/*.py` shows all 44 modules already carry a `[TODAY]`/`[EXPERIMENTAL]`/`[FUTURE]` tier label, every one in the module docstring/header (first occurrence <= line 40, verified for all 44). Example: `bin/bash.py:15` reads `UmerOS /bin/bash - The Bourne Again Shell  [TODAY]`. The mandatory tier-label rule (§4.4, line 120) is therefore satisfied repo-wide for `bin/`. Rolled out during the per-file baseline work (H29/H32). No code change needed; bookkeeping only.
- [x] H41 | YELLOW | `build/UmerOS-GUI.spec:5` | **RESOLVED (session 60):** the PyInstaller spec froze `ui/umeros_gui.py` — the retired legacy PyQt6 desktop shell (H25 superseded it with Flutter `ui/flutter_ui/`, H11/§4.8). Repointed `_ENTRY` to `ui/launch_gui.py`, the thin Python host that boots `flutter_ui` (the canonical Flutter frontend). Added `[FIX H41]` note; the spec still fails closed on a missing entry. Also hardened `launch_gui.py:main()` to default to Desktop on `EOFError` so the frozen `console=False` binary launches without a TTY. The standard's "Tkinter" label was imprecise (actual toolkit is PyQt6). Both files compile clean.
- [x] H43 | YELLOW | `build/UmerOS-GUI.spec:5` | **RESOLVED (session 61):** drift-recon proved the premise STALE — the standard cited a hardcoded absolute Windows path `UmerOS\ui\umeros_gui.py`. The spec no longer contains any absolute drive path (grep: 0 matches for `[A-Z]:\\`); `_ENTRY` is derived repo-relatively via `_REPO_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(SPEC)))` + `os.path.join(_REPO_ROOT, "ui", "launch_gui.py")`. The `[FIX H43]` note (lines 15-16) documents this, and the H42 session-32 rewrite already recorded "hardcoded dev path removed ([FIX H43])". Build is portable/reproducible. No code change needed; bookkeeping only.
- [x] H44 | YELLOW | `build/UmerOS-GUI.spec:8-15` + repo root | **RESOLVED (session 62):** two fixes. (1) Spec: `optimize=0`->`optimize=1` (smaller/faster bytecode) and added `[FIX H44]` note — `datas`/`binaries`/`hiddenimports` are intentionally empty because `_ENTRY` (`ui/launch_gui.py`) is a thin stdlib-only Flutter-launching host; the Flutter app is bundled separately via `flutter build`, so PyInstaller has nothing extra to collect (verified: `launch_gui.py` uses no dynamic imports). (2) Codified the build: new `build/build_umeros_gui.py` runs `PyInstaller UmerOS-GUI.spec` then the mandatory `sign_artifact.py` signing gate (fail-closed; `UMEROS_ALLOW_UNSIGNED=1` dev opt-out). `ci.yml` does not build the GUI, so this script is the codified, reproducible, signed-artifact release job. Note: actual freeze/clean-VM run not executed here (no PyInstaller/Flutter in sandbox) — manual follow-up.
- [x] H47 | YELLOW | `cloud/ota_updater/update_system.py` (whole module) | **RESOLVED (session 64):** brought to the §4.4 per-file baseline — `from __future__ import annotations` added, `print` replaced with `logging` (module logger `UmerOS.Cloud.OtaUpdater.update_system`), full type hints, Google-style docstrings, and `[EXPERIMENTAL]` tier label (the module is simulated). `run_update_pipeline` wrapped in try/except; the `verify_and_apply` fail-closed posture (H46/H154) is unchanged. The standard's 'no try/except' half was overstated (verify already guarded the crypto boundary).
- [x] H48 | YELLOW | `cloud/ota_updater/update_system.py` | Hardcoded `update_url = "https://updates.umeros.dev/latest"` — a simulated domain baked into code; no pinned/v
- [x] H49 | YELLOW | `cloud/ota_updater/update_system.py` | **Misleading docs / overstated security.** Docstring claims "Verify cryptographic signature" and "Uses the Cry
- [x] H50 | YELLOW | `compatibility/container_engine.py:43` | `GPL-3.0 (GNU General Public License Version 3)` header in `container_engine.py` — contradicts canonical GPL-3.0 (extends H7/H20/H30). Th **RESOLVED (session 67):** `container_engine.py:43` `Version 3`->`v3` (canonical GPL-3.0 per H7); normalized `Licence: GPL-3.0`->`License: GPL-3.0` across all 29 British-spelling files (the 'add headers for consistency' intent, scaled to the real 29 — the folder now has 33 .py files, all header-bearing, so the 'other 3 files have no header' premise was STALE). `container.py`/`syscall_shim.py` already use the full GPL boilerplate; `__init__.py` a short comment — both left canonical. Zero `Licence:` spellings remain; all 30 changed files `py_compile` clean. [FIX H50].
- [x] H52 | YELLOW | `compatibility/container_engine.py:365,445,547 (+ ContainerEngine.launch facade :659)` | Foreign binaries launched **unsandboxed.** `LinuxCompat.launch` (L269) / `WineShim.run` (L348) / `AndroidConta **RESOLVED (session 68):** all three foreign-binary launchers (`LinuxCompat.launch` :365, `WineShim.run` :445, `AndroidContainer.launch_app` :547) plus the `ContainerEngine.launch` facade (:659) now call `_require_launch_capability()` (fail-closed `gate.require("container.launch")` when a CapabilityManager is wired or strict mode) and no longer inherit the *full* parent env - `_sanitize_env()` passes an allow-listed subset, blocking leakage of trust material (UMEROS_OTA_*/OPENROUTER_API_KEY). Opt-in POSIX sandbox (`sandbox=True` or UMEROS_COMPAT_SANDBOX=1) applies `unshare` (new mount/PID/net ns) + `resource.setrlimit` quotas via preexec_fn; refused fail-closed when requested but unavailable (non-POSIX). `gate.enforcing` refuses any unsandboxed launch outright. Full chroot/OCI/seccomp remains the module FUTURE note. [FIX H52].
- [x] H53 | YELLOW | `compatibility/container.py` + `compatibility/syscall_shim.py` (both f | These two files skip the per-file baseline: `print` instead of `logging`, no `from __future__ import annotatio **RESOLVED (session 69):** container.py + syscall_shim.py brought to the §4.4 baseline: added `from __future__ import annotations`, replaced all `print()` with the module `log` logger (dotted `UmerOS.Compat.*`), added Google-style docstrings on every method, and a `[TODAY]` tier label. The `syscall_table` dynamic dispatch (syscall_shim.py) was already a closed allow-list of trusted `_umer_*` handlers; the rewrite keeps it closed (lookup via `.get()`, never a user-supplied callable). Both files `py_compile` clean and a smoke test confirms logging is used (no print), the fail-closed gate denies execution without the HARDWARE capability, and unimplemented syscalls return None. [FIX H53].
...
- [x] H56 | YELLOW | `core/command.py:25-28` | The base `Command` declares a `privileges: List[str]` field ("Required privileges (e.g. ["user"], ["root"])") **RESOLVED (session 71):** added a fail-closed `run()` / `check_privileges()` privilege gate to the base `Command` (H55 `execute()` contract left untouched for `bin/`/`kernel/` callers + tests). `run(args, has_privilege)` denies (returns 77 / EX_NOPERM) when `check_privileges` fails; unverifiable privileged commands (no `has_privilege` verifier wired) are denied by default, never run un-gated. `has_privilege` is a `Callable[[str], bool]` seam the kernel wires to `CapabilityManager` / `Credentials` (integration follow-up, cf. H114/H115). `core/command.py` `py_compile` clean; `tests/test_command.py` -> 6 passed; enforcement smoke OK. [FIX H56].
- [x] H59 | YELLOW | `dev/core.py:69` (`DeviceNode.mode` default) + `dev/*_device.py` | The base `DeviceNode` dataclass defaults to `mode=0o666` (world **read+write**), and a long list of device nod **RESOLVED (session 73):** drift-recon showed `dev/` was already mostly disciplined (defaults 0o660/0o640, only a few stray 0o666). Real gaps closed: (1) base `DeviceNode.mode` default 0o666 -> 0o640 (owner rw, group r) in `dev/core.py`; (2) `makedev.py`/`mknod_virtual.py` helper `create_device` default perms 0o666 -> 0o640; (3) privileged pseudo/misc nodes (tty/ptmx/tun/fuse/i2c/misc-char) tightened 0o666 -> 0o660; (4) classic data devices (null/zero/full/log/shm/vsock) kept 0o666 but made explicit + justified (Unix-norm world-rw for non-security data sinks). All edits carry `# [FIX H59]`. No test asserts device modes (grep 0 hits). `py_compile` clean across all 21 edited `dev/` files; smoke `DeviceNode().mode == 0o640` OK. [FIX H59].
- [x] H60 | YELLOW | `dev/core.py:211-237` (`DeviceManager.sync_to_filesystem`) | **Privileged VFS mutation with no capability gate.** `sync_to_filesystem()` calls `os.mknod`/`os.mkfifo`/`os.s **RESOLVED (session 74):** `dev/core.py` `DeviceManager.sync_to_filesystem()` now gates device-node materialization fail-closed behind `CAP_SYS_ADMIN` via the process-global capability gate (zero-trust family H27/H28/H46/H51) — `gate.require(CAP_SYS_ADMIN)` raises PermissionError when no trust source is wired + strict, else permissive-with-warning (matches the gate's contract). Added a CWE-22 dev-root confinement: every node path is resolved and proved a descendant of `self.dev_root`; a node that would escape the UmerOS virtual `/dev` namespace is skipped (REFUSED, never materialized), so sync can never write onto a real host `/dev` unless authorized. Lazy `from core.capability_gate import gate, CAP_SYS_ADMIN` (no import cycle). New `tests/test_dev_manager.py` (3 cases: fail-closed under strict; permissive default; escape-node refused) all pass. `py_compile` clean. [FIX H60].
...
- [ ] H133 | YELLOW | `legal/consent.py:35` | **`DEFAULT_LEDGER_PATH` is a hardcoded developer Windows path** (`UmerOS/var/lib/ume
...
- [x] H45 | BLUE | `build/UmerOS-GUI.spec:1`, `build/__init__.py` | **RESOLVED (session 63):** added canonical GPL-3.0 license header + `SPDX-License-Identifier: GPL-3.0-or-later` to `build/UmerOS-GUI.spec` (the `[TODAY]` tier label was already present at line 3, so the tier-label half of the premise was stale; only the license header was genuinely missing). `build/__init__.py` left as the intentional 0-byte package marker (harmless).
- [x] H54 | BLUE | `compatibility/container.py` (`ZeroTrustContainer`) vs `compatibility/ | Two divergent "container" models: the real, used `ContainerEngine`/`ContainerInstance`, and the unused `ZeroTr **RESOLVED (session 70):** drift-recon corrected the premise - `ZeroTrustContainer` is NOT a dead stub: it is exported by `compatibility/__init__.py`, has a dedicated real test (`tests/test_zero_trust_container.py`, 5 cases, all green), and is the kernel's intended hardware-gated execution path (the kernel wiring in `umer_kernel.py` is currently commented out). The two models are *complementary*, not duplicate: `ZeroTrustContainer` enforces `HARDWARE` (fail-closed, H51) for single-binary hardware access via `SyscallShim`; `ContainerEngine` enforces `container.launch` (fail-closed, H52) for multi-backend foreign-binary launch via `SyscallTranslator`. Both are zero-trust. Fix: documented the intentional separation + a 'do not merge their gates/shims' warning in `compatibility/__init__.py` (stop-gap containers note) and added a clarifying module docstring to `container.py`. No deletion/merge (would break the test + kernel intent). [FIX H54].
- [x] H57 | BLUE | `core/command.py` (whole module) | No tier label (should be `[TODAY]` — it is the live command base) and no license header (consistent with `bin/ **RESOLVED (session 72):** added the missing `[TODAY]` tier label (`# [TODAY]` module comment after the import block, matching the `syscall_shim.py` convention) to `core/command.py`. Drift-recon corrected the premise: the 'no license header' half was STALE - the file already carries the full canonical GPL-3.0 boilerplate (lines 1-12); only the `[TODAY]` tier label was genuinely missing. `py_compile` clean; `tests/test_command.py` -> 6 passed (H55/H56 contract preserved). [FIX H57].
...
- [x] H122 | BLUE | `kernel/gui.py` | **Kivy (Tkinter-style) GUI prototype inside `kernel/`** — contradicts the Flutter-canonical decision (H11/H25)
...
- [x] H125 | BLUE | `kernel/requirements.txt` | **Dependency drift** — lists `kivy>=2.2.0` (not Flutter, H11/H15) + `cryptography` (unused: the live `CryptoEn
...
## NEXT — YELLOW sweep (session 41→45): H7(39)/H11(40)/H13(41)/H14(42)/H15(43)/H16(44)/**H19 RESOLVED (session 45)** — `ai/` consolidation: drift-recon proved premise substantially stale. `assistant.py`=`LocalAIAssistant` shim; `self_healing.py`=H12/H21-gated wrapper over `SelfHealingEngine` (security, not dup); `resource_predictor.py:ResourcePredictor` had ZERO importers, duplicated canonical `AIResourceManager` → converted to deprecated alias (removed ~356 lines dead logic; one predictor chosen). `ai/umer_ai.py` is the single source of truth. **H20 RESOLVED (session 46)** — `ai/` GPL license labels: drift-recon showed `ai/providers.py:43` was ALREADY canonical `License: GPLv3` (no British `Licence` spelling anywhere in `ai/`); 3 outliers (`umer_ai.py:36`, `resource_predictor.py:34`, `__init__.py:17`) used long-form `License: GPL-3.0 (GNU General Public License Version 3)` → normalized all 3 to `License: GPLv3`; `ai/` now 9/9 consistent. **H23 RESOLVED (session 47)** — `ui/flutter_ui` app registry: drift-recon proved premise STALE — `app_registry.dart` already the single source of truth (`AppRegistry`); consumed by `desktop_shell.dart` (`_DesktopGrid` L474/`_GlobalSearchModal` L1011/`_LaunchPad` L1139) + `dock.dart` (3 refs); invariants in `test/app_registry_test.dart`; `AppDefinition(` appears only there, no duplicate lists. **H24 RESOLVED (session 48)** — `ui/flutter_ui` Power/Idle naming: drift-recon proved premise STALE — the cited "CPUIdle & Governor" (`:287`) and "Power & Idle" (`:448`) strings do NOT exist in `ui/flutter_ui` (0 hits). Launcher title is plain `Power & Performance` (single source of truth `app_registry.dart:114`, asserted by `test/app_registry_test.dart:41`); `desktop_shell.dart:325` popup matches. Only jargon hit (`power_governor_app.dart:106` "CPUIdle & Power Governor Framework") is an inside-app section header naming the real subsystem — accurate, not a launcher label. **H26 RESOLVED (session 49)** — `ui/flutter_ui` a11y (REAL fix, not stale): added `Semantics` labels/button roles to every custom `GestureDetector`/`InkWell` control — in `dock.dart` (app icons + window buttons), `draggable_window.dart` (focus + header drag + 8 resize handles), and `desktop_shell.dart` (menubar app entries, date/time status, spotlight app icons, desktop-grid items, LaunchPad + global-search dismiss layers, LaunchPad grid items). Broadened `test/widget_test.dart` with a static-label assertion for the date/time control. `flutter analyze` green (0 new issues; 2 pre-existing unrelated info-lints remain). NOTE: `flutter test` is blocked in this sandbox — native-asset resolution needs MSVC/cl.exe (Visual Studio) which is not installed; tests will run in a proper env. **H30 RESOLVED (session 50)** — `boot/` GPL-3.0 header strays (H7 variant, REAL): drift-recon confirmed premise live — 10 `boot/` modules declared `License: GPL-3.0 (GNU General Public License Version 3)`. Normalized per H7 session-39 rule (`Version 3`→`v3`): all 10 now read `License: GPL-3.0 (GNU General Public License v3)` (American `License` already correct; no `Licence`→`License` needed). All 10 byte-compile clean. **H31 RESOLVED (session 51)** — `boot/` tier labels (§4.4, REAL fix): drift-recon showed premise PARTLY STALE — `bootloader.py` (L152/172/176) AND `efi_system.py` (L544) already carried inline `[TODAY]`, but NO module-level tier label existed in `boot/`. Added module-level `[TODAY]` to 22 modules + `[EXPERIMENTAL]` to `demo_boot.py`; added docstring+label to `boot/__init__.py` (no prior docstring). Verified: all 23 `boot/*.py` carry a module-level label; 0 compile failures. **H32 RESOLVED (session 52)** — `boot/uefi_stub.c` (C placeholder) + `boot/init.py` ctypes claim: premise CONFIRMED (overstatement, not a hard bug). `uefi_stub.c` = documented `[FUTURE]` scaffold (cited as pseudocode by `bootloader.py:25`), NOT compiled/linked (setup.py builds 0 C extensions), NO `ctypes`/`CDLL` bridge (grep: only a `(Simulated)` print). Fix per §9: (1) `uefi_stub.c` header relabeled `[FUTURE]` non-functional (removed false "interface with UEFI via embedded CPython/Cython" claim); (2) `init.py:71` no longer falsely claims "via ctypes" → "UEFI hardware layer not wired (placeholder scaffold only)" + `[FIX H32]`; (3) added `from __future__ import annotations` to `init.py` (per-file baseline, standard L121). `init.py` byte-compiles clean. Next: **H33** — `boot/bootloader.py:153` (SHA3-256) vs `boot/efi_system.py:82` (SHA-256): inconsistent hashing vs §4.2 design mandate. Say **'continues'** for H33.
Say **'continues'** for H7.

## NEXT (where to pick up) — dev/ sweep (session 72)
**H36 RESOLVED (session 56):** code verification confirmed `_exec_command` (L182–211) and `_exec_shell` (L213–226) already gated — no code change needed; bookkeeping only.
**H48 RESOLVED (session 65):** `cloud/ota_updater/update_system.py` — the simulated `update_url` was hardcoded in code (no config surface, no cert/key pinning). Externalized to `UMEROS_OTA_UPDATE_URL` (default `DEFAULT_OTA_UPDATE_URL`, also constructor-injectable); server cert pinned via `UMEROS_OTA_SERVER_CERT_FP` + expected host via `UMEROS_OTA_PINNED_HOST`; new `_assert_update_source_trusted()` enforces HTTPS-only, refuses loopback/link-local hosts, cert-host match, and a pinned signing key — all fail-closed. [FIX H48].
**H49 RESOLVED (session 66):** `cloud/ota_updater/update_system.py` — drift-recon showed the standard's "self-signs, fakes PASS" premise was already stale (H46/H154 made verification fail-closed); the live overstatements were the module docstring ("Simulates a **secure** OTA pipeline" + garbled "Marked the module is production update client.") and the class docstring ("**Secure** OTA update service"). Corrected both to honest [EXPERIMENTAL] wording that names the real fail-closed signature boundary and labels the transport/rollback/audit gaps; added a real in-memory `_audit_log` (`_audit`/`get_audit_log`) + a simulated one-way `rollback()` (records `applied_version`/`last_good_version` on apply). [FIX H49].
**H59 RESOLVED (session 73):** `dev/` DeviceNode world-writable default 0o666 fixed — base default 0o640, mknod helpers 0o640, privileged pseudo nodes (tty/ptmx/tun/fuse/i2c/misc) 0o660, classic data devices (null/zero/full/log/shm/vsock) kept 0o666 but now explicit+justified. 21 files `py_compile` clean; smoke `DeviceNode().mode==0o640` OK.
**H60 RESOLVED (session 74):** `dev/core.py` `DeviceManager.sync_to_filesystem()` gated fail-closed behind `CAP_SYS_ADMIN` (zero-trust family) + every node confined to `dev_root` (CWE-22) so it cannot write onto a real host `/dev`. 3-test regression suite added; `py_compile` clean.
Next: **H61** — `dev/` (`DeviceManager` singleton registry lock). Say **'continues'** for H61.
