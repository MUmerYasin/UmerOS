...
## NEXT — YELLOW sweep (session 39): H7 RESOLVED (license consistency — the "Apache-2.0 strays" premise was stale drift; doc scope already declares GPL-3.0 canonical; normalized `Licence`→`License` + `Version 3`→`v3` to match README). H9 done (session 38). Next: **H11** — UI tech reconciliation: Kivy in code/README/setup.py vs decided **Flutter (Dart)** canonical (2026-08-20); retire/migrate `ui/*.py` (H25), drop `kivy` from `requirements.txt` (H15), reconcile blueprint + README "Project Structure" drift (H14). Say **'continues'** for H11.
