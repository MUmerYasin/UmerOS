...
## NEXT — YELLOW sweep (session 41): H7 RESOLVED (39), H11 RESOLVED (40), **H13 RESOLVED (session 41)** — `.umerpkg` was overstated crypto (claimed "Signed" but only SHA3-256 integrity hash, no signature/trust); now real Ed25519 signature over the integrity hash + pinned chain-of-trust anchor in `packages/trusted_keys.py` (canonical `umer-release` public key; private key offline), `_verify_package` fail-closed refuses unsigned/untrusted, 4 regression tests added. H14/H15/H116 still open (broader scope: README layout drift / g4f+floating pins / capability-gate+Flutter launch). Next: **H14** — README layout drift. Say **'continues'** for H14.
