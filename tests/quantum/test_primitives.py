...
from quantum.operators import SparsePauliOp, pauli_string
...
class TestPrimitiveJob:
    ...
    def test_status(self):
        # [RECONCILE] status is a @property returning a PrimitiveJobStatus enum
        job = PrimitiveJob(lambda: 42)
        _ = job.result
        status = job.status
        assert status == PrimitiveJobStatus.DONE
...
class TestSamplerV2:
    ...
    def test_bell_state_sampling(self):
        ...
        qc.h(0)
        qc.cx(0, 1)
        # [RECONCILE] shipped measure() takes (qubit: int, classical_bit: int)
        qc.measure(0, 0)
        qc.measure(1, 1)
        sampler = SamplerV2()
...
class TestEstimatorV2:
    ...
    def test_z_expectation(self):
        ...
        job = estimator.run(circuits=[qc], observables=[obs])
        result = job.result
        # [RECONCILE] EstimatorPubResult exposes the value via .data["evs"]
        ev = result.quasi_dists[0].data["evs"]
        np.testing.assert_allclose(ev, 1.0, atol=1e-6)

    def test_x_expectation(self):
        qc = QuantumCircuit(1)
        qc.h(0)
        obs = pauli_string("X", 1.0)
        estimator = EstimatorV2()
        job = estimator.run(circuits=[qc], observables=[obs])
        result = job.result
        ev = result.quasi_dists[0].data["evs"]
